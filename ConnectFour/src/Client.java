...
/** similar to the lobby, give each player their own game lobby running on its own thread
 * between threads pass a data structure containing the game and a flag. When the flag is set play X can move and when it is unset
 * player Y can move. The server should check should verify what is being sent between clients
	...
	public void newGameroomWithPlayer(String command) {
		try {
			...
			if (response.contains("does not exist") || response.contains("is unavailable")) {
				...
				//get the second player and dump them in the same gameloop 
				// or else let each play have their own game loop? but each game has to be sent back and forth from player to player
				//reach accross to the other thread someonhow and grab p2 to dump them in the gameloop??? how to reach accross threads?
				// --> create the gameloop then send the other player the gameLoop() command and automatically run it?
				// --> create the gameloop then send the other player the gameLoop() command and automatically run it?
			/*UserInputThread uiThread = new UserInputThread(socket, bufferedReader, inputStream, outputStream, cl);
			uiThread.run();*/
				gameLoop();
